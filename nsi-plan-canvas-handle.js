(function () {
  ...
  window.beginCanvasResize = function (event) {
    ...
    state.planCanvasResizeDrag = {
      ...
      startCanvasX: state.planCanvas.x || 0,
      startCanvasY: state.planCanvas.y || 0
    };
    document.addEventListener('pointermove', moveCanvasResize);
    document.addEventListener('pointerup', endCanvasResize, { once: true });
  };

  function moveCanvasResize(event) {
    const drag = state.planCanvasResizeDrag;
    if (!drag) return;
    state.planCanvas.w = clamp(drag.startW + event.clientX - drag.startX, 720, 3600);
    state.planCanvas.h = clamp(drag.startH + event.clientY - drag.startY, 480, 1800);
  ...
  function endCanvasResize() {
    document.removeEventListener('pointermove', moveCanvasResize);
    ...
    return `
      <div class="plan enhanced-plan ${state.planEditing ? 'plan-editing' : ''}">
        ...
        <div class="plan-caption">
          <div><strong>${escapeHtml(currentFloor?.name || 'Этаж')}</strong><span>${rooms.length} зон и помещений · окно и белое поле меняются за уголки</span></div>
        ...
        <div class="plan-window-scroll simple-viewport">
          <div class="plan-map detailed layered free-canvas" style="${viewportStyle()}" onpointerdown="beginPlanCanvasDrag(event)">
            <div class="plan-canvas resizable-canvas" style="${canvasStyle()}">
              <div class="plan-core core-a">Лифты / лестница</div>
              <div class="plan-core core-b">Инженерный стояк</div>
              ${tiles}
              <span class="canvas-resize-handle" onpointerdown="beginCanvasResize(event)" title="Изменить размер белого поля"></span>
            </div>
